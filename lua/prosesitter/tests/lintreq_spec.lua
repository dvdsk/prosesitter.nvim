...
describe("lintreq spacing", function()
	it("single row", function()
		...
		lr:add(buf, 0, 0, 0+#"each ")
		lr:add(buf, 0, 6, 6+#"word ")
		lr:add(buf, 0, 12, 12+#"a lintreq ")
        local req = lr:build()
...
describe("lintreq duplication", function()
    ...
    it("basic", function()
        ...
        lr:add(buf, 1, 4 + 1, 4+1+#"hello how are you") -- one space extra
        local req = lr:build()
    ...
    it("overwrite part of lintreq", function()
        ...
        lr:add(buf, 0, 4, 4 + #"hi")
		lr:add_append_text(buf, 0, " ")
        lr:add(buf, 1, 4 + 1, 4+1+#"hello how are you")
